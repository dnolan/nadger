unit srcNamedColours;

interface

uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs,
  ExtCtrls, StdCtrls, srcMain;
...
type
  TfrmNamedColours = class(TForm)
    imgColours: TImage;
    hintColourName: TStaticText;
    Label1: TLabel;
    procedure FormCreate(Sender: TObject);
    procedure imgColoursMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure imgColoursClick(Sender: TObject);
  private
...
var
  frmNamedColours: TfrmNamedColours;
  HexValue : String;
...
procedure TfrmNamedColours.FormCreate(Sender: TObject);
var
   tmpRect : TRect;
   I, L, T, ColorSize, CubeSize, Gap : Integer;
...
begin
   ...
   begin
      ...
      for I := 1 to 455 do
      begin
      	tmpRect := Rect(L, T, L+ColorSize, T+ColorSize);
	      Canvas.Brush.Color := HYPEColors[I];
	      Canvas.FillRect(tmpRect);
         L := L + ColorSize+Gap;
         if L = CubeSize then
         begin
...
procedure TfrmNamedColours.imgColoursMouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
var
	Col, Row, Colour : Integer;
begin
   Col := (X div 12) + 1;
   Row := (Y div 12);

   Colour := (Row*22)+Col;
   HexValue := IntToHex(HYPEColors[Colour], 6);
   HexValue := Copy(HexValue, 5, 2)+Copy(HexValue, 3, 2)+Copy(HexValue, 1, 2);
   
   if Colour <= 455 then
	   hintColourName.Caption := HYPEColorNames[Colour] + ' (#' + HexValue + ')'
   else
   	hintColourName.Caption := '';
end;

procedure TfrmNamedColours.imgColoursClick(Sender: TObject);

begin
   frmMain.Hex.HexFull := HexValue;
   frmMain.SetFormValues(True, True, True);
   Hide;
end;
